...
│  任务类型 → 引擎映射：                               │
│  ┌────────────────────┬─────────────────────────┬──────────────────────────┐ │ 
│  │ 气弹耦合+荷载生成  │ OpenFAST (Python)       │ 输出六分量内力(Fx,Fy,Fz, │ │
│  │                    │                         │ Mx,My,Mz) → SAP2000      │ │
│  │ 结构设计验算       │ SAP2000 OAPI (.NET)     │ 输入六分量内力，完成校核  │ │
│  │ 地震/非线性分析    │ OpenSeesPy (Python)     │ 时程分析 / Pushover      │ │
│  │ 疲劳损伤计算       │ Python 计算 + 后处理    │ 基于SAP2000内力结果的疲劳 │ │
│  └────────────────────┴─────────────────────────┴──────────────────────────┘ │
└────────────────────────────────────────────────────┘
```
...
SAP2000 (结构设计验算)
  ...
  │
  ▼
OpenSeesPy (可选 — 抗震/非线性专项分析)
  │  地震时程分析 / Pushover / 非线性屈曲
  │  输出：抗震性能指标
```

> **关键设计决策**：OpenFAST 只负责生成荷载（六分量内力），不参与结构校核。SAP2000 只做结构验算，不自己做气弹分析。两个软件通过六分量内力这个"接口"解耦，各自专注自己最擅长的部分。OpenSeesPy 作为补充，用于抗震和非线性分析。
